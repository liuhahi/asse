...
class A1:
    ...
    @staticmethod
    def display_guess_matrix(
        guess_no: int, word_length: int, scores: Tuple[int, ...]
    ) -> None:
        pass

    @staticmethod
    def compute_value_for_guess(
        word: str, start_index: int, end_index: int, guess: str
    ) -> int:
        pass
...
class TestDesign(TestA1):
    ...
    def test_clean_import(self):
        ...
        self.assertIsCleanImport(
            self.a1, msg="You should not be printing on import for a1.py"
        )
    ...
    def test_functions_defined(self):
        ...
        for func_name, func in inspect.getmembers(A1, predicate=inspect.isfunction):
            ...
            self.aggregate(
                self.assertFunctionDefined, a1, func_name, num_params, tag=func_name
            )
...
@skipIfFailed(
    ...
    tag=A1.select_word_at_random.__name__,
)
...
@skipIfFailed(
    TestDesign,
    TestDesign.test_functions_defined.__name__,
    tag=A1.create_guess_line.__name__,
)
...
@skipIfFailed(
    TestDesign,
    TestDesign.test_functions_defined.__name__,
    tag=A1.display_guess_matrix.__name__,
)
...
@skipIfFailed(
    TestDesign,
    TestDesign.test_functions_defined.__name__,
    tag=A1.compute_value_for_guess.__name__,
)
...
@skipIfFailed(
    TestDesign, TestDesign.test_functions_defined.__name__, tag=A1.main.__name__
)
class TestMain(TestFunctionality):
    ...
    def assertMain(self, file_in: str, file_out: str, stop_early: bool = False):
        ...
        expected, result, stdio = self._run_main(
            file_in, file_out, stop_early=stop_early
        )
        self.assertMultiLineEqual(stdio.stdinout, expected)
        if stdio.stdin != "":
            self.fail(msg="Not all input was read")
        self.assertIsNone(
            result, msg="main function should not return a non None value"
        )
    ...
    def test_main_fixed_invalid_length(self):
        ...
        self.set_seed()
        self.assertMain(
            "main_fixed_invalid_length.in", "main_fixed_invalid_length.out"
        )  # jousting
...
def main():
    ...
    master = TestMaster(
        ...
        scripts=[("a1", "a1.py"), ("a1_support", "a1_support.py")],
    )
